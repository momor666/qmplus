                  ...
                  JOIN (
                           SELECT DISTINCT ra.userid
                            ...
                            WHERE ra.roleid IN ($this->gradebookroles)
                              AND ra.contextid " . $this->get_related_contexts_string($this->context) . "
...
// Element is a category
                    ...
                    //DH - added from grader report - hides categories with no grade item
...
// Element is a grade_item
    ...
    public function process_action($target, $action) {
        ...
        debugging('This function is has been updated to static do_process_action below, following grade book.',
            DEBUG_DEVELOPER);
    ...
    protected static function get_collapsed_preferences($courseid) {
        if ($collapsed = get_user_preferences('grade_report_marking_collapsed_categories'.$courseid)) {
        ...
        if (($oldcollapsedpref = get_user_preferences('grade_report_marking_collapsed_categories')) !== null) {
            if ($collapsedall = unserialize_array($oldcollapsedpref)) {
                ...
                if (!empty($collapsed['aggregatesonly']) || !empty($collapsed['gradesonly'])) {
                    ...
                    if (!empty($collapsedall['aggregatesonly']) || !empty($collapsedall['gradesonly'])) {
                        set_user_preference('grade_report_marking_collapsed_categories', serialize($collapsedall));
                    } else {
                        unset_user_preference('grade_report_marking_collapsed_categories');
                    }
                }
            } else {
    ...
    protected static function set_collapsed_preferences($courseid, $collapsed) {
        ...
        if (!empty($collapsed['aggregatesonly']) || !empty($collapsed['gradesonly'])) {
            set_user_preference('grade_report_marking_collapsed_categories'.$courseid, json_encode($collapsed));
        } else {
            unset_user_preference('grade_report_marking_collapsed_categories'.$courseid);
        }
    }
	...
	public function get_sort_arrows(array $extrafields = array()) {
        ...
        if ($this->sortitemid === 'username') {
            if ($this->sortorder == 'ASC') {
                $arrows['username'] .= $OUTPUT->pix_icon('t/sort_asc', $strsortasc, '', array('class' => 'iconsmall sorticon'));
            } else {
                $arrows['username'] .= $OUTPUT->pix_icon('t/sort_desc', $strsortdesc, '', array('class' => 'iconsmall sorticon'));
            }
        }
    ...
    /**
     *
     * DH copied this so marking report keeps working
     * copeid from depreciatedlib 2.8.7
     ...
     * @return string
     */
    public function get_related_contexts_string(context $context) {
