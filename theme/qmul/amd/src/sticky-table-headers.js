...
function StickyTableHeader(options, scrollableElement) {
  ...
  function _$tableSetter(tableElement) {
    ...
    if (_$tableHeader) {
      _$tableHeaderClone = _$tableHeader.cloneNode(true);
      _$tableHeaderClone.classList.add('clone');

      _$table.insertBefore( _$tableHeaderClone, _$tableHeader.nextSibling);

      _setTableHeaderClone.call(this, _$table.querySelector('thead.clone'));
    }
  }
  ...
  function _setScrollableElementWithElement(scrollableElement) {
    ...
    if (_$scrollable != null) {
      _sth.helpers.removeEventListener(window, 'scroll', _onChange);
      _sth.helpers.removeEventListener(window, 'resize', _onChange);
    }
    ...
    if (_$scrollable != null) {
      _sth.helpers.addEventListener(window, 'scroll', _onChange);
  ...
  /** @private */
  function _setTableHeaderClone(tableHeaderCloneElement) {
    _$tableHeaderClone = tableHeaderCloneElement;
  }
  ...
  function _onChangeHandler() {
    ...
    if (!_$tableHeaderClone.classList.contains('use')) {
      _$tableHeaderClone.classList.add('use');
    }

    var captionExists = (_$tableCaption != null);

    var tablePositionTop = this.$table.getBoundingClientRect().top;
    var tablePositionBottom = this.$table.getBoundingClientRect().bottom;
    ...
    var scrollableElementBorderTopWidth = 0;
    var scrollableElementTop            = document.querySelector('.navbar.fixed-top').offsetHeight;
    var scrollingThreshold              = tablePositionTop;
    var scrollingThresholdBottom        = tablePositionBottom;
    ...
    if (scrollingThreshold < scrollableElementTop && scrollingThresholdBottom > scrollableElementTop) {
      ...
      _$tableBody = _$table.querySelector('tbody');
      _$tableHeader.style.maxWidth = getComputedStyle(_$tableBody).getPropertyValue('width');
    ...
    _cleanupStickyElement(_$tableHeaderClone);
  ...
  function _processTableHeaderCells(tableHeaderCells) {
    var tableBodyFirstRow             = this.$table.querySelector('tbody > tr:first-child');
    var tableBodyFirstRowCells        = this.$table.querySelectorAll('tbody > tr:first-child > *');
    var n = 2;

    if (tableBodyFirstRow.classList.contains('plugintypeheader') || tableBodyFirstRow.classList.contains('rolecapheading')) {
      tableBodyFirstRowCells = this.$table.querySelectorAll('tbody > tr:nth-child(2) > *');
    }

    while (tableBodyFirstRowCells.length == 0) {
      tableBodyFirstRowCells        = this.$table.querySelectorAll('tbody > tr:nth-child('+n+') > *');
      n++;
      if (n == 10) {
        tableBodyFirstRowCells = null;
      }
    }
    ...
    for (var i = 0; i < tableHeaderCells.length; i++) {
      ...
      var expectedNewWidth = tableBodyFirstRowCells[i].getBoundingClientRect().width;
