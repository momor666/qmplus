...
class theme_qmul_gender {
    ...
    protected $metaphone_function = null;

    public function __construct($Females_ref=null, $Males_ref=null) {
      ...
      if (!$Females_ref || !$Males_ref) {
          ...
          $this->Males = arsort($Males_ref);
          $this->Females = arsort($Females_ref);
      }

      if (is_callable('double_metaphone')) {
          $this->metaphone_function = 'double_metaphone';
      } else {
          $this->metaphone_function = 'metaphone';
      }
    }

    public function gender($name, $looseness=6) {
      ...
      for ($i = 0; $i < $looseness; $i++) {
          if (!isset($this->MATCH_LIST[$i]))
              continue;

          $function = $this->MATCH_LIST[$i];
          $gender = $this->$function($name);
    ...
    private function one_only($name) {

      $gender = null;

      // Match one list only

      $male_hit = isset($this->Males[$name]);
      $female_hit = isset($this->Females[$name]);
    ...
    private function either_weight($name) {

      $gender = null;

      // Match either, weight

      $male_hit = isset($this->Males[$name])? $this->Males[$name] : 0;
      $female_hit = isset($this->Females[$name])? $this->Females[$name] : 0;

      if (($female_hit>0) || ($male_hit>0)) {
          $gender = ($female_hit > $male_hit) ? 'female' : 'male';
      }

      return $gender;
    }

    private function one_only_metaphone($name) {

      $gender = null;

      // Match one list only, use DoubleMetaphone

      $metaphonefunc = $this->metaphone_function;
      $meta_name = $metaphonefunc($name);
      $metaphone_hit = '';
      ...
      foreach ($this->Females as $list_name => $weight)
      {
          if ($female_hit>0)
              break;

          $meta_list_name = $metaphonefunc($list_name);
      ...
      foreach ($this->Males as $list_name => $weight)
      {
          if ($male_hit>0)
              break;
    ...
    private function either_weight_metaphone($name) {
    ...
    private function v2_rules($name) {
      $gender = null;
      ...
      // jon and john
      if (preg_match('/^joh?n/', $name)) { $gender = 'male'; }
      // tom and thomas and tomas and toby
      else if (preg_match('/^th?o(m|b)/', $name)) { $gender = 'male'; }
      else if (preg_match('/^frank/', $name)) { $gender = 'male'; }
      else if (preg_match('/^bil/', $name)) { $gender = 'male'; }
      else if (preg_match('/^hans/', $name)) { $gender = 'male'; }
      else if (preg_match('/^ron/', $name)) { $gender = 'male'; }
      else if (preg_match('/^ro(z|s)/', $name)) { $gender = 'female'; }
      else if (preg_match('/^walt/', $name)) { $gender = 'male'; }
      else if (preg_match('/^krishna/', $name)) { $gender = 'male'; }
      else if (preg_match('/^tri(c|sh)/', $name)) { $gender = 'female'; }
      // pascal and pasqual
      else if (preg_match('/^pas(c|qu)al$/', $name)) { $gender = 'male'; }
      else if (preg_match('/^ellie/', $name)) { $gender = 'female'; }
      else if (preg_match('/^anfernee/', $name)) { $gender = 'male'; }

      return $gender;
    }

    private function v1_rules($name) {
      $gender = null;

      // Match using rules from v0.10 of Text::GenderFromName

      // most names ending in a/e/i/y are female
      if (preg_match('/^.*[aeiy]$/', $name)) $gender = 'female';
      // allison and variations
      if (preg_match('/^all?[iy]((ss?)|z)on$/', $name)) $gender = 'female';
      // cathleen, eileen, maureen
      if (preg_match('/een$/', $name)) $gender = 'female';
      // barry, larry, perry
      if (preg_match('/^[^s].*r[rv]e?y?$/', $name)) $gender = 'male';
      // clive, dave, steve
      if (preg_match('/^[^g].*v[ei]$/', $name)) $gender = 'male';
      // carolyn, gwendolyn, vivian
      if (preg_match('/^[^bd].*(b[iy]|y|via)nn?$/', $name)) $gender = 'female';
      // dewey, stanley, wesley
      if (preg_match('/^[^ajklmnp][^o][^eit]*([glrsw]ey|lie)$/', $name)) $gender = 'male';
      // heather, ruth, velvet
      if (preg_match('/^[^gksw].*(th|lv)(e[rt])?$/', $name)) $gender = 'female';
      // gregory, jeremy, zachary
      if (preg_match('/^[cgjwz][^o][^dnt]*y$/', $name)) $gender = 'male';
      // leroy, murray, roy
      if (preg_match('/^.*[rlr][abo]y$/', $name)) $gender = 'male';
      // abigail, jill, lillian
      if (preg_match('/^[aehjl].*il.*$/', $name)) $gender = 'female';
      // janet, jennifer, joan
      if (preg_match('/^.*[jj](o|o?[ae]a?n.*)$/', $name)) $gender = 'female';
      // duane, eugene, rene
      if (preg_match('/^.*[grguw][ae]y?ne$/', $name)) $gender = 'male';
      // fleur, lauren, muriel
      if (preg_match('/^[flm].*ur(.*[^eotuy])?$/', $name)) $gender = 'female';
      // lance, quincy, vince
      if (preg_match('/^[clmqtv].*[^dl][in]c.*[ey]$/', $name)) $gender = 'male';
      // margaret, marylou, miri;
      if (preg_match('/^m[aei]r[^tv].*([^cklnos]|([^o]n))$/', $name)) $gender = 'female';
      // clyde, kyle, pascale
      if (preg_match('/^.*[ay][dl]e$/', $name)) $gender = 'male';
      // blake, luke, mi;
      if (preg_match('/^[^o]*ke$/', $name)) $gender = 'male';
      // carol, karen, shar;
      if (preg_match('/^[cks]h?(ar[^lst]|ry).+$/', $name)) $gender = 'female';
      // pam, pearl, rachel
      if (preg_match('/^[pr]e?a([^dfju]|qu)*[lm]$/', $name)) $gender = 'female';
      // annacarol, leann, ruthann
      if (preg_match('/^.*[aa]nn.*$/', $name)) $gender = 'female';
      // deborah, leah, sarah
      if (preg_match('/^.*[^cio]ag?h$/', $name)) $gender = 'female';
      // frances, megan, susan
      if (preg_match('/^[^ek].*[grsz]h?an(ces)?$/', $name)) $gender = 'female';
      // ethel, helen, gretchen
      if (preg_match('/^[^p]*([hh]e|[ee][lt])[^s]*[ey].*[^t]$/', $name)) $gender = 'female';
      // george, joshua, theodore
      if (preg_match('/^[^el].*o(rg?|sh?)?(e|ua)$/', $name)) $gender = 'male';
      // delores, doris, precious
      if (preg_match('/^[dp][eo]?[lr].*s$/', $name)) $gender = 'female';
      // anthony, henry, rodney
      if (preg_match('/^[^jpswz].*[denor]n.*y$/', $name)) $gender = 'male';
      // karin, kim, kristin
      if (preg_match('/^k[^v]*i.*[mns]$/', $name)) $gender = 'female';
      // bradley, brady, bruce
      if (preg_match('/^br[aou][cd].*[ey]$/', $name)) $gender = 'male';
      // agnes, alexis, glynis
      if (preg_match('/^[acgk].*[deinx][^aor]s$/', $name)) $gender = 'female';
      // ignace, lee, wallace
      if (preg_match('/^[ilw][aeg][^ir]*e$/', $name)) $gender = 'male';
      // juliet, mildred, millicent
      if (preg_match('/^[^agw][iu][gl].*[drt]$/', $name)) $gender = 'female';
      // ari, bela, ira
      if (preg_match('/^[abeiuy][euz]?[blr][aeiy]$/', $name)) $gender = 'male';
      // iris, lois, phyllis
      if (preg_match('/^[egilp][^eu]*i[ds]$/', $name)) $gender = 'female';
      // randy, timothy, tony
      if (preg_match('/^[art][^r]*[dhn]e?y$/', $name)) $gender = 'male';
      // beatriz, bridget, harriet
      if (preg_match('/^[bhl].*i.*[rtxz]$/', $name)) $gender = 'female';
      // antoine, jerome, tyrone
      if (preg_match('/^.*oi?[mn]e$/', $name)) $gender = 'male';
      // danny, demetri, dondi
      if (preg_match('/^d.*[mnw].*[iy]$/', $name)) $gender = 'male';
      // pete, serge, shane
      if (preg_match('/^[^bg](e[rst]|ha)[^il]*e$/', $name)) $gender = 'male';
      // angel, gail, isabel
      if (preg_match('/^[adfgim][^r]*([bg]e[lr]|il|wn)$/', $name)) $gender = 'female';

      return $gender;
    }

    protected $Males = array(
