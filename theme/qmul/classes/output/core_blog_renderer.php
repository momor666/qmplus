 ...
 * Blog renderer
 */
class theme_qmul_core_blog_renderer extends core_blog_renderer {
	...
	public function render_blog_entry(blog_entry $entry) {
        ...
        // Commands.
        $o .= $this->output->container_start('commands btn-group btn-group-sm my-1');
        if ($entry->renderable->usercanedit) {
            ...
            if (empty($entry->uniquehash)) {
                $o .= html_writer::link(new moodle_url('/blog/edit.php',
                                                        ...
                                                        $stredit, array('class'=>'btn btn-secondary'));
            }
            $o .= html_writer::link(new moodle_url('/blog/edit.php',
                                                    array('action' => 'delete', 'entryid' => $entry->id)),
                                                    $strdelete, array('class'=>'btn btn-secondary'));
        }

        $entryurl = new moodle_url('/blog/index.php', array('entryid' => $entry->id));
        $o .= html_writer::link($entryurl, get_string('permalink', 'blog'), array('class'=>'btn btn-secondary'));
