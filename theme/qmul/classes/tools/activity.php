...
use \theme_qmul\tools\activity_meta;
...
class activity {
    ...
    public static function module_meta(\cm_info $mod) {
        ...
        if (method_exists('theme_qmul\tools\activity', $methodname)) {
            $meta = call_user_func('theme_qmul\tools\activity::' . $methodname, $mod);
        } else {
    ...
    protected static function std_meta(\cm_info $mod,
        ...
        $meta->submittedstr = get_string($submitstrkey, 'theme_qmul');
        $meta->notsubmittedstr = get_string('not'.$submitstrkey, 'theme_qmul');

        // If module is not visible to the user then don't bother getting meta data.
        // SYNERGY - 11th December 2017 - Ignoring this behaviour for Pivotal ticket https://www.pivotaltracker.com/story/show/152641771
        //if (!$mod->uservisible) {
            //return $meta;
        //}
        ...
        if (has_capability('mod/assign:grade', $mod->context)) {
            ...
            if (method_exists('theme_qmul\tools\activity', $methodnsubmissions)) {
                $meta->numsubmissions = call_user_func('theme_qmul\tools\activity::'.$methodnsubmissions, $courseid, $mod->instance);
            }
            if (method_exists('theme_qmul\tools\activity', $methodnungraded)) {
                $meta->numrequiregrading = call_user_func('theme_qmul\tools\activity::'.$methodnungraded, $courseid, $mod->instance);
            }
        } else {
    ...
    public static function assign_ungraded($courseids, $since = null) {
        ...
        foreach ($courseids as $courseid) {
            ...
            $sql = "SELECT cm.id AS coursemoduleid, a.id AS instanceid, a.course,
    ...
    public static function quiz_ungraded($courseids, $since = null) {
        ...
        foreach ($courseids as $courseid) {
            ...
            $sql = "SELECT cm.id AS coursemoduleid, q.id AS instanceid, q.course,
    ...
    public static function assign_num_submissions_ungraded($courseid, $modid) {
        ...
        $sql = "SELECT sb.assignment, count(sb.userid) AS total
    ...
    protected static function std_num_submissions($courseid,
        ...
        if (!isset($modtotalsbyid[$maintable][$courseid])) {
            ...
            $sql = "SELECT m.id, COUNT(DISTINCT sb.userid) as totalsubmitted
                      FROM {".$maintable."} m
    ...
    public static function assign_num_submissions($courseid, $modid) {
        ...
        if (!isset($modtotalsbyid['assign'][$courseid])) {
            ...
            $sql = "SELECT m.id, COUNT(sb.userid) as totalsubmitted
                  FROM {assign} m
                  JOIN {assign_submission} sb
    ...
    public static function quiz_num_submissions_ungraded($courseid, $quizid) {
        ...
        if (!isset($totalsbyquizid)) {
            ...
            $sql = "SELECT q.id, count(DISTINCT qa.userid) as total
                      FROM {quiz} q
					  JOIN {quiz_attempts} qa ON qa.quiz = q.id
					   AND qa.sumgrades IS NULL
                     WHERE qa.userid NOT IN ($graderids)
    ...
    public static function get_submission_row($courseid, $mod, $submissiontable, $modfield, $extraselect='') {
        ...
        if ($mod->modname === 'assign') {
            ...
            $sql = "SELECT a.id AS instanceid, st.*
    ...
    public static function instance_activity_dates($courseid, $mod, $timeopenfld = '', $timeclosefld = '') {
        ...
        if (!isset($moddates[$courseid . '_' . $mod->modname][$mod->instance]) || PHPUNIT_TEST) {
            ...
            if ($mod->modname === 'assign') {
                $timeopenfld = 'allowsubmissionsfromdate';
                $timeclosefld = 'duedate';
            }
            $sql = "SELECT
                    module.id,
                    module.$timeopenfld AS timeopen,
                    module.$timeclosefld AS timeclose";
            if ($mod->modname === 'quiz' || $mod->modname === 'lesson' || $mod->modname === 'assign') {
                $id = $mod->modname === 'quiz' ? $mod->modname : $mod->modname.'id';
                ...
                if ($groups[0]) {
                    ...
                    $groupbysql = " GROUP BY module.id, timeopen, timeclose, ovrd1.$timeopenfld, ovrd1.$timeclosefld";
                } else {
    ...
    public static function grade_row($courseid, $mod) {
        ...
        $sql = "SELECT m.id AS instanceid, gg.*
