...
class theme_qmul_core_enrol_renderer extends core_enrol_renderer {
    ...
    public function user_roles_and_actions($userid, $roles, $assignableroles, $canassign, $pageurl) {
        ...
        foreach ($roles as $roleid => $role) {
            if ($canassign and (is_siteadmin() or isset($assignableroles[$roleid])) and !$role['unchangeable']) {
                ...
                $icon = html_writer::empty_tag('img', array('class'=>'hidden', 'alt' => $strunassign, 'src' => $iconenrolremove));
                $icon .= html_writer::tag('i', '', array('class'=>'glyphicon glyphicon-remove role-remove'));
                ...
                $rolesoutput .= html_writer::tag('div', $role['text'] . html_writer::link($url, $icon,
                    ...
                    array('class' => 'tag tag-pill tag-primary role role_'.$roleid));
            } else {
                $rolesoutput .= html_writer::tag('div', $role['text'], array('class' => 'tag tag-pill tag-default role unchangeable', 'rel' => $roleid));
            }
        }
        $output = '';
        $output .= html_writer::tag('div', $rolesoutput, array('class' => 'roles col-md-6'));
        if (!empty($assignableroles) && $canassign) {
            ...
            if (!$hasallroles) {
                ...
                $output .= html_writer::tag('div', html_writer::link($url, get_string('assignroles', 'role'),
                    array('class' => 'btn btn-outline-primary btn-sm assignrolelink', 'title' => get_string('assignroles', 'role'))),
                    array('class' => 'addrole col-md-6'));
            }
        }
        $output = html_writer::tag('div', $output, array('class'=>'d-flex'));
        return $output;
    }
    ...
    public function user_groups_and_actions($userid, $groups, $allgroups, $canmanagegroups, $pageurl) {
        ...
        foreach ($groups as $groupid => $name) {
            if ($canmanagegroups and groups_remove_member_allowed($groupid, $userid)) {
                $icon = html_writer::empty_tag('img', array('class'=>'hidden', 'alt' => get_string('removefromgroup', 'group', $name), 'src' => $iconenrolremove));
                $icon .= html_writer::tag('i', '', array('class'=>'glyphicon glyphicon-remove role-remove'));
                ...
                $groupoutput .= html_writer::tag('div', $name . html_writer::link($url, $icon),
                    array('class' => 'group label', 'rel' => $groupid));
            } else {
                $groupoutput .= html_writer::tag('div', $name, array('class' => 'group', 'rel' => $groupid));
            }
        }
        $groupoutput = html_writer::tag('div', $groupoutput, array('class' => 'groups col-md-6'));
        if ($canmanagegroups && (count($groups) < count($allgroups))) {
            $icon = get_string('add');
            $url = new moodle_url($pageurl, array('action' => 'addmember', 'user' => $userid));
            $btn = html_writer::tag('div', html_writer::link($url, $icon), array('class' => 'btn btn-sm btn-outline-secondary'));
            $groupoutput .= html_writer::tag('div', $btn, array('class'=>'col-md-6 addgroup'));
        }
        $groupoutput = html_writer::tag('div', $groupoutput, array('class'=>'d-flex'));
        return $groupoutput;
    }
}