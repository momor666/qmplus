...
class newBlacklisted extends \core\task\scheduled_task {


    public function get_name() {
        // Shown in admin screens
        return get_string('notifyadmins2', 'report_qmplus');
    }
    ...
    public function execute() {
        ...
        $newFiles = report_qmplus_getBlacklistedFiles($date->getTimestamp());

        if($newFiles->valid()){

            $users = report_qmplus_getSiteAdmins(); // get users to send email to
            $file = report_qmplus_createEmailReportBlacklisted($newFiles);
            $sender = null;
            ...
            foreach ($users as $user) {
                ...
                $send[]=report_qmplus_sendBlackListedEmail($sender, $user, $file);
            ...
            if($externalUsers!=''){  // if external users exist, send them an email
                ...
                foreach ($externalUsers as $user) {
                    $externalUsersSend[]=report_qmplus_sendBlackListedEmail($sender, $user, $file);
                }
            }
