...
//Get list of possible export formats
      $mform->addElement('select','format',get_string('format'), $this->get_download_menu());
...
//Put the groups in to name order and create drop down
    ...
    function get_download_menu() {
        ...
        foreach ($allclasses as $class) {
            $matches = array();
            if (preg_match('/^ilp\_table\_([a-z]+)\_export\_format$/', $class, $matches)) {
                $type = $matches[1];
