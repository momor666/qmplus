 ...
 * @param object $block
 */
function xmldb_block_ulcc_diagnostics_upgrade($oldversion, $block) {
    global $CFG, $DB;

    $result = TRUE;
    $dbman = $DB->get_manager();

    if ($oldversion < 2011040301) {

        // Define table ulcc_admin_flags to be created
        $table = new xmldb_table('ulcc_admin_flags');

        // Adding fields to table ulcc_admin_flags
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('itemid', XMLDB_TYPE_INTEGER, '10', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, null);
        $table->add_field('tablename', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, '0');
        $table->add_field('externalid', XMLDB_TYPE_CHAR, '255', null, XMLDB_NOTNULL, null, '0');

        // Adding keys to table ulcc_admin_flags
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Conditionally launch create table for ulcc_admin_flags
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // ulcc_diagnostics savepoint reached
        upgrade_block_savepoint(true, 2011040300, 'ulcc_diagnostics');
    }
