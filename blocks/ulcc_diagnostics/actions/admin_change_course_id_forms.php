...
class admin_change_course_id_form extends abstract_multiple_form
{
    function definition()
    {
    ...
    {
        global $CFG;
        $return_url = "$CFG->wwwroot/blocks/ulcc_diagnostics/actions/course_rollover.php";

        $data = $this->get_data();
        if (!$data) {
            echo $OUTPUT->heading('Bulk Upload Courses');
            echo '<p>Format: old_id, new_id</p>';
            echo '<p>This form will swap old ID numbers for new ID numbers only. New courses should be uploaded separately.</p>';
            $this->display();
...
class admin_change_course_id_intermediate extends abstract_multiple_form
{
    function definition()
    {
        global $CFG;
    ...
    private function process_Row(csv_import_reader $csv_importer, form_upload_progress_tracker $upload_tracker, moodle_database $DB, $STD_FIELDS, $columns, $courses)
    {
        ...
        while ($line = $csv_importer->next()) {
            ...
            $existing_course = $DB->get_record('course', array('idnumber' => $course_rec->old_id));
            if (!$existing_course) {
                $upload_tracker->track('created', 'Course does not exist: ' . $course_rec->fullname . ' - ' . $course_rec->shortname);
            }else {
            	$existing_course->idnumber = $course_rec->new_id;
                $DB->update_record('course', $existing_course);
                $upload_tracker->track('created', 'Course idnumber updated: ' . $course_rec->old_id . ' - ' . $existing_course->idnumber.' for Moodle ID '.$existing_course->id);
            }
    ...
    function validation($data, $files)
    {
        ...
        if (!in_array('old_id', $columns)) {
            $errors['tutor'] = get_string('missingfield', 'error', 'oldid');
        }
        if (!in_array('new_id', $columns)) {
            $errors['student'] = get_string('missingfield', 'error', 'newid');
        }
        return $errors;
    }
}

class admin_change_course_id_form_final extends abstract_multiple_form
{
