...
class block_reportsdash_targetbycourse_filter_form extends moodleform
{
    function definition() {
        global $DB,$CFG;
        ...
        if(empty($imports->singlemode))
        {
            ...
            $items = $DB->get_records_sql('select value FROM {block_ilp_plu_ste_items} items');
            $select = $mform->addElement('select', 'target_criteria', get_string('target_criteria', $L));
