 ...
 * Version details
 *
 * This file has been updated by merging the latest commit (1dfc28c640a31319981fa4d81156929902f88c7a)
 * of the master brunch of the ULCC version and the the ULCC-QMUL version, master brunch,
 * commit 4ec92c3abce63985b6a28143129def363e065906, moodle-auth_dbsyncother2 repo.
 *
 *  Kept the best logic of both files
 *
 * @package    auth
...
class auth_plugin_dbsyncother extends auth_plugin_base {
    ...
    function auth_plugin_dbsyncother() {
        ...
        //$this->authtype = 'dbsyncother';  // This is wrong as ignore 'syncauth' setting and causing update problems
        $this->authtype = isset($this->config->syncauth) ? $this->config->syncauth : 'shibboleth'; // It should use the config setting ($config->syncauth = shibboleth)
        if (empty($this->config->extencoding)) {
            $this->config->extencoding = 'utf-8';
        }
        if( isset( $CFG->mis_host ) ){
            $this->config->host = $CFG->mis_host;
            $this->config->mis_host = $CFG->mis_host;
        }
        if( isset( $CFG->mis_dbase ) ){
            $this->config->name = $CFG->mis_dbase;
            $this->config->mis_dbase = $CFG->mis_dbase;
        }
        if( isset( $CFG->mis_user ) ){
            $this->config->user = $CFG->mis_user;
            $this->config->mis_user = $CFG->mis_user;
        }
        if( isset( $CFG->mis_pass ) ){
            $this->config->pass = $CFG->mis_pass;
            $this->config->mis_pass = $CFG->mis_pass;
        }
    }
    /**
    ...
    function db_init() {
        ...
        // check for common configuration to the MIS first
        if(isset($this->config->mis_host) && isset($this->config->mis_dbase) && isset($this->config->mis_user) && isset($this->config->mis_pass) ){
            $authdb->Connect($this->config->mis_host, $this->config->mis_user, $this->config->mis_pass, $this->config->mis_dbase, true);
        } else {
        ...
        if ($selectfields) {
            ...
            if ($rs) {
                if ( !$rs->EOF ) {
                    ...
                    foreach ($selectfields as $localname => $externalname) {
                        //$result[$localname] = $fields_obj->{$localname};  // ULCC version
                        $result[$localname] = core_text::convert($fields_obj->{$localname}, $this->config->extencoding, 'utf-8'); // ULCC-QMUL version
                    }
                }
                $rs->Close();
            }
    ...
    function sync_users($do_updates=false, $verbose=false) {
        ...
        if (!empty($this->config->removeuser)) {
            ...
            if (count($userlist)) {
                ...
                /*  ULCC VERSION,
                    Do need or as setting is applied in the constructor
                    $sql = "SELECT u.*
                          FROM {user} u
                         WHERE (u.auth = :authtype OR u.auth = :syncauth)
                         AND u.deleted=0";
                */
                // ULCC-QMUL Version
                $sql = "SELECT u.*
                ...
                $params = array();
                $params['authtype'] = $this->authtype;
                // $params['syncauth'] = $this->config->syncauth; // ULCC Version. Do need this anymore
            }
            $remove_users = $DB->get_records_sql($sql, $params);
            if (!empty($remove_users)) {
                // Kept the ULCC version of counting deleted users
                $count=0;
                foreach ($remove_users as $user) {
                    ...
                    /*
                     Commented out the ULCC version and added the ULCC-QMUL version. This is a matter of preference.
                    I (Panos) recommend to keep ULCC version.
        ...
        if ($do_updates) {
            ...
            if (!empty($updatekeys)) {
                ...
                $params['authtype'] = $this->authtype;
                //$params['syncauth'] = $this->config->syncauth; // ULCC Version. This is not needed anymore
                /*
                 ULCC version
                $sql = "SELECT u.id, u.username
                          FROM {user} u
                         WHERE (u.auth = :authype OR u.auth = :syncauth)
                           AND u.username {$in_sql}";
                */
                $sql = "SELECT u.id, u.username
                          FROM {user} u
        ...
        // Create missing accounts. We need to check for those that have been created with 'dbsyncother' as well as
        // those that are for the intended auth type.
        /*
          ULCC Version
        $sql = "SELECT u.username
                 ...
                 WHERE (u.auth = :authype OR u.auth = :syncauth)
                   AND u.deleted = 0
                   AND u.suspended = 0 ";
        $params = array('authtype' => $this->authtype,
            'syncauth' => $this->config->syncauth);
        */
        /*
         *  Small refactoring of the ULCC-QMUL Version
         */
        $sql = "SELECT u.username
                  FROM {user} u
                 WHERE u.auth = :authtype AND u.deleted = 0";
        $params = array('authtype' => $this->authtype);
        // End of Small refactoring
        /*
           ULCC version
        $add_users = array_diff($userlist, $DB->get_fieldset_sql($sql, $params));
        */
        // ULCC-QMUL version
            ...
            foreach ($updatekeys as $key) {
                ...
                if (!empty($this->config->{'field_updatelocal_' . $key})) {
                    ...
                    */
                     ...
                     * ULCC-QMUL Version
                     */
                    // Always compare original and new field values as strings
                    if (isset($user->{$key})) { // only update the value if the original variable was set
                        $orig_value = $user->{$key};
                        $different = "$orig_value" !== "$value";
                        if($different) { // only update if it's changed
                            $DB->set_field('user', $key, $value, array('id'=>$userid));
                            $updated = true;
                        }
                    }
    ...
    function can_change_password() {
        return ($this->is_internal() or isset($this->config->changepasswordurl)/*!empty($this->config->changepasswordurl)*/);
    }
    /**
