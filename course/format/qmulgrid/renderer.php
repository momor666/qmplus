...
class format_qmulgrid_renderer extends format_grid_renderer {
    ...
    protected $portable = 0; // 1 = mobile, 2 = tablet.
    ...
    public function __construct(moodle_page $page, $target) {
        ...
        /* Since format_qmulgrid_renderer::section_edit_controls() only displays the 'Set current section' control when editing
    ...
    protected function page_title() {
        return get_string('sectionname', 'format_qmulgrid');
    }
    ...
    public function print_multiple_section_page($course, $sections, $mods, $modnames, $modnamesused) {
        ...
        echo html_writer::start_tag('div', array('id' => 'gridiconcontainer', 'role' => 'navigation',
            'aria-label' => get_string('gridimagecontainer', 'format_qmulgrid')));
        ...
        $defaultcustommousepointers = get_config('format_qmulgrid', 'defaultcustommousepointers');
        ...
        if (!(($course->coursedisplay == COURSE_DISPLAY_MULTIPAGE) && (!$editing))) {
            ...
            echo html_writer::start_tag('div', array('id' => 'gridshadebox_content', 'class' => implode(' ',
                ...
                'role' => 'region',
                'aria-label' => get_string('shadeboxcontent', 'format_qmulgrid')));
            ...
            echo html_writer::tag('img', '', array('id' => 'gridshadebox_close', 'style' => 'display: none;',
                'class' => $deviceextra,
                'src' => $this->output->pix_url('close', 'format_qmulgrid'),
                'role' => 'link',
                'aria-label' => get_string('closeshadebox', 'format_qmulgrid')));
            ...
            if (($course->numsections > 1) || (($course->numsections == 1) && (!$this->topic0attop))) {
                ...
                $prev = html_writer::start_tag('div', array('id' => 'gridshadebox_previous',
                    ...
                    'style' => 'display: none;',
                    'role' => 'link',
                    'aria-label' => get_string('previoussection', 'format_qmulgrid')));
                $prev .= html_writer::tag('img', '', array('class' => 'gridshadebox_arrow gridshadebox_previous'.$deviceextra,
                    'src' => $this->output->pix_url('fa-arrow-circle-'.$previcon.'-w', 'format_qmulgrid')));
                ...
                $next = html_writer::start_tag('div', array('id' => 'gridshadebox_next',
                    ...
                    'style' => 'display: none;',
                    'role' => 'link',
                    'aria-label' => get_string('nextsection', 'format_qmulgrid')));
                $next .= html_writer::tag('img', '', array('class' => 'gridshadebox_arrow gridshadebox_next'.$deviceextra,
                    'src' => $this->output->pix_url('fa-arrow-circle-'.$nexticon.'-w', 'format_qmulgrid')));
        ...
        $PAGE->requires->js_init_call('M.format_qmulgrid.init', array(
        ...
        $PAGE->requires->yui_module('moodle-format_qmulgrid-gridkeys', 'M.format_qmulgrid.gridkeys.init',
    ...
    protected function section_edit_control_items($course, $section, $onsectionpage = false) {
        ...
        if (!$isstealth && $section->section && has_capability('moodle/course:setcurrentsection', $coursecontext)) {
            if ($course->marker == $section->section) {  // Show the "light globe" on/off.
                $url->param('marker', 0);
                $markedthissection = get_string('markedthissection', 'format_qmulgrid');
                ...
                $markthissection = get_string('markthissection', 'format_qmulgrid');
    ...
    private function make_block_topic0($course, $sections, $modinfo, $editing, $urlpicedit, $streditsummary,
        ...
        if ($editing) {
            ...
            if ($this->topic0attop) {
                $strhidesummary = get_string('hide_summary', 'format_qmulgrid');
                $strhidesummaryalt = get_string('hide_summary_alt', 'format_qmulgrid');

                echo html_writer::link(
                        $this->courseformat->qmulgrid_moodle_url('mod_summary.php', array(
                            'sesskey' => sesskey(),
                            'course' => $course->id,
                            'showsummary' => 0)), html_writer::empty_tag('img', array(
                            'src' => $this->output->pix_url('into_grid', 'format_qmulgrid'),
    ...
    private function make_block_icon_topics($contextid, $modinfo, $course, $editing, $hascapvishidsect,
        ...
        if ($this->settings['newactivity'] == 2) {
            ...
            if (!file_exists("$CFG->dirroot/course/format/qmulgrid/pix/new_activity_".$currentlanguage.".png")) {
                $currentlanguage = 'en';
            }
            $urlpicnewactivity = $this->output->pix_url('new_activity_'.$currentlanguage, 'format_qmulgrid');
        ...
        if ($this->settings['showsectiontitlesummary'] == 2) {
            global $PAGE;
            $PAGE->requires->js_call_amd('format_qmulgrid/tooltip', 'init', array());
        }
    ...
    protected function output_section_image($section, $sectionname, $sectionimage, $contextid, $thissection, $gridimagepath) {
        ...
        if (is_object($sectionimage) && ($sectionimage->displayedimageindex > 0)) {
            ...
            $imgurl = $this->output->pix_url('info', 'format_qmulgrid');
            $content = html_writer::empty_tag('img', array(
                'src' => $imgurl,
                'alt' => $sectionname,
                'class' => 'info',
    ...
    private function make_block_icon_topics_editing($thissection, $contextid, $urlpicedit) {
        ...
        $streditimage = get_string('editimage', 'format_qmulgrid');
        $streditimagealt = get_string('editimage_alt', 'format_qmulgrid');

        echo html_writer::link(
            $this->courseformat->qmulgrid_moodle_url('editimage.php', array(
    ...
    private function make_block_icon_topic0_editing($course) {
            $strdisplaysummary = get_string('display_summary', 'format_qmulgrid');
            $strdisplaysummaryalt = get_string('display_summary_alt', 'format_qmulgrid');
        ...
        echo html_writer::link(
            $this->courseformat->grid_moodle_url('mod_summary.php', array(
                ...
                html_writer::empty_tag('img', array(
                    'src' => $this->output->pix_url('out_of_grid', 'format_qmulgrid'),
    ...
    private function make_block_topics($course, $sections, $modinfo, $editing, $hascapvishidsect, $streditsummary,
        ...
        for ($section = 1; $section <= $course->numsections; $section++) {
            ...
            if ($hascapvishidsect || ($thissection->visible && $thissection->available)) {
                ...
                echo html_writer::tag('p', get_string('hidden_topic', 'format_qmulgrid'));
