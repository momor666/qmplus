 ...
 */
class format_qmulgrid_observer {
    ...
    public static function course_content_deleted(\core\event\course_content_deleted $event) {
        if (class_exists('format_qmulgrid', false)) {
            // If class format_qmulgrid was never loaded, this is definitely not a course in 'Grid' format.
            global $DB;
            /* Delete any images associated with the course.
               ...
               it is not now. */
            $courseformat = format_qmulgrid::get_instance($event->objectid);
            $courseformat->delete_images();
            unset($courseformat);  // Destruct.

            $DB->delete_records("format_qmulgrid_summary", array("courseid" => $event->objectid));
        }
    }
}
