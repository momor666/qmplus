...
function xmldb_format_qmulgrid_upgrade($oldversion = 0) {
    global $DB;
    ...
    if ($oldversion < 2012011701) {
        ...
        if ($dbman->table_exists('course_grid_icon')) {
            ...
            if (!$dbman->table_exists('format_qmulgrid_icon')) {
                $dbman->rename_table($table, 'format_qmulgrid_icon');
            } else {
        ...
        if ($dbman->table_exists('course_grid_summary')) {
            $table = new xmldb_table('course_grid_summary');
            if (!$dbman->table_exists('format_qmulgrid_summary')) {
                $dbman->rename_table($table, 'format_qmulgrid_summary');
            } else {
                // May as well tidy up the db.
                $dbman->drop_table($table);
            }
    ...
    if ($oldversion < 2012071500) {
        $table = new xmldb_table('format_qmulgrid_summary');
        ...
        $table = new xmldb_table('format_qmulgrid_icon');
