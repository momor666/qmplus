...
YUI.add('moodle-format_qmulgrid-gridkeys', function (Y, NAME) {
...
M.format_qmulgrid.gridkeys = M.format_qmulgrid.gridkeys || {};
M.format_qmulgrid.gridkeys = {
    ...
    findfocused: function() {
        ...
        M.format_qmulgrid.gridkeys.currentGridBox = false;
        if (focused && focused.id) {
            Y.log('Focus id: ' + focused.id);
            if (focused.id.indexOf('gridsection-') > -1) {
                Y.log('Grid id: ' + focused.id);
                M.format_qmulgrid.gridkeys.currentGridBox = true;
                M.format_qmulgrid.gridkeys.currentGridBoxIndex = parseInt(focused.id.replace("gridsection-", ""), 10);
            }
        }
        return M.format_qmulgrid.gridkeys.currentGridBox;
    },
    init: function(params) {
        Y.on('esc', function (e) {
            e.preventDefault();
            Y.log("Esc pressed");
            Y.log("Selected section no: " + M.format_qmulgrid.selected_section_no);
            M.format_qmulgrid.icon_toggle(e);
        });
        // Initiated in CONTRIB-3240...
        Y.on('enter', function (e) {
            if (M.format_qmulgrid.gridkeys.currentGridBox) {
                e.preventDefault();
                if (e.shiftKey) {
        ...
        Y.on('tab', function (/*e*/) {
            setTimeout(function() {
                ...
                if (M.format_qmulgrid.gridkeys.findfocused()) {
                    M.format_qmulgrid.tab(M.format_qmulgrid.gridkeys.currentGridBoxIndex);
                }
            }, 250);
        });
        ...
        Y.on('left', function (e) {
            ...
            Y.log("Left pressed");
            if (params.rtl) {
                M.format_qmulgrid.next_section(e);
            } else {
                M.format_qmulgrid.previous_section(e);
            }
        });
