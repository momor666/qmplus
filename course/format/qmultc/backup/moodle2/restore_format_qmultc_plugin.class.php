 ...
 * needed to restore one qmultc course format.
 */
class restore_format_qmultc_plugin extends restore_format_plugin {
    ...
    protected function define_course_plugin_structure() {
        ...
        $elename = 'qmultc'; // This defines the postfix of 'process_*' below.
        $elepath = $this->get_pathfor('/'); // This is defines the nested tag within 'plugin_format_qmultc_course' to allow
                                            // '/course/plugin_format_qmultc_course' in the path therefore as a path structure
    ...
    /**
     * Process the 'plugin_format_qmultc_course' element within the 'course' element in the 'course.xml' file in the
     * '/course' folder of the zipped backup 'mbz' file.
     */
    public function process_qmultc($data) {
        global $DB;

        $data = (object) $data;

        // We only process this information if the course we are restoring to
        // has 'qmultc' format (target format can change depending of restore options).
        ...
        if (empty($data->layoutcolumns)) {
            ...
            $data->layoutcolumns = get_config('format_qmultc', 'defaultlayoutcolumns');
        }

        $courseformat->restore_qmultc_setting(
            $data->courseid,
            $data->layoutelement,
     ...
     */
    public function process_format_qmultc($data) {
        global $DB;
