 ...
 */
function required_qmultc_param($parname) {
    if (empty($parname)) {
        throw new coding_exception('required_qmultc_param() requires $parname to be specified');
    }
    $param = required_param($parname, PARAM_RAW);

    return clean_qmultc_param($param);
}

/**
 * Used by required_qmultc_param to clean the toggle parameter.
 *
 * @param string $param the variable we are cleaning
 * @return mixed
 * @throws coding_exception
 */
function clean_qmultc_param($param) {
    if (is_array($param)) {
        throw new coding_exception('clean_qmultc_param() can not process arrays.');
    } else if (is_object($param)) {
        if (method_exists($param, '__toString')) {
            $param = $param->__toString();
        } else {
            throw new coding_exception('clean_qmultc_param() can not process objects.');
        }
    }
